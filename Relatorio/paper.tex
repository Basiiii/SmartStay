...
\begin{itemize}
	...
	\item \textbf{Métodos de Teste}: \texttt{MethodName\_Condition\_ExpectedOutcome}
\end{itemize}
...
Para uniformizar as operações de gestão, o sistema utiliza a interface \textit{ManageableEntity}, implementada pelas classes de coleção (\textit{Clients}, \textit{Owners}, \textit{Reservations} e \textit{Accommodations}). Esta interface define métodos comuns, como \texttt{Add()}, \texttt{Remove()}, \texttt{Export()} e \texttt{Load()}.
...
\begin{itemize}
	\item \textbf{Eficiência de Espaço}: Como o Protobuf é um formato binário, é muito mais compacto em comparação ao JSON, o que resulta em menos espaço de armazenamento.
...
O desenvolvimento da interface da aplicação foi realizado utilizando a linguagem de programação \textit{Dart} e a \textit{framework} \textit{Flutter}, uma combinação que oferece uma abordagem eficiente para a construção de interfaces de utilizador modernas e reativas. O \textit{Flutter} é uma framework de \textit{UI} de código aberto que permite o desenvolvimento de aplicações nativas para várias plataformas com uma única base de código.  

\subsection{Estado Atual da Aplicação}
\label{subsec:estadoatual}

A aplicação desenvolvida encontra-se num estado preliminar, servindo como um exemplo de implementação prática do uso de controladores (\textit{controllers}) e \textit{web requests}. Não é uma aplicação completa, mas sim um modelo demonstrativo, cujo objetivo principal é ilustrar a interação básica entre uma interface de utilizador e um backend.  
...
\subsection{Interação com Controladores e HTTP Requests}
\label{subsec:http-requests}
...
\begin{lstlisting}[
	language=Java, 
	caption={Botão para iniciar sessão como cliente},
	label={lst:button-client-login}]
// Botão de exemplo - Iniciar Sessão como Cliente
Expanded(
...
O botão acima está configurado para chamar a função \texttt{\_handleCreateClient()} ao ser pressionado. Esta função realiza as seguintes etapas:  

\begin{itemize}
	\item \textbf{Recolha de Dados}: Obtém os valores inseridos pelo utilizador nos controladores de texto.
	\item \textbf{Validação}: Verifica se os dados recolhidos são válidos (por exemplo, se nenhum campo obrigatório está vazio).
	\item \textbf{Pedido HTTP}: Envia os dados para o backend, utilizando a função \texttt{createBasicClient()}, que realiza a interação com o controlador.
\end{itemize}

Segue-se a implementação da função \texttt{\_handleCreateClient()}, que realiza estas operações:

\begin{lstlisting}[
	language=Java, 
	caption={Função que processa os dados e faz a chamada HTTP},
	label={lst:handle-create-client}]
Future<bool> _handleCreateClient() async {
	...
	if (result != null) {
		...
		if (kDebugMode) {
			print('Cliente criado com sucesso: $result');
		}
		return true;
	} else {
...
Nesta função, a chamada HTTP é realizada por \texttt{createBasicClient()}, que comunica diretamente com o controlador do backend. Esta abordagem modular permite que a lógica de validação e comunicação seja isolada em funções separadas, facilitando a manutenção e a escalabilidade do projeto.  

Segue-se agora a implementação da função \texttt{createBasicClient()}, que faz o pedido HTTP para o backend. Ela é responsável por enviar os dados do cliente para a API e retornar a resposta do servidor:

\begin{lstlisting}[
	language=Java, 
	caption={Função que realiza a requisição HTTP para criar um cliente},
	label={lst:create-basic-client}]
Future<Map<String, dynamic>?> createBasicClient({
	...
	try {
		// Fazer uma requisição POST para o endpoint /client/basic
		...
		if (response.statusCode == 201) {
			// Retornar os dados do cliente recém-criado
			return response.data;
		} else {
			// Tratar códigos de status inesperados
			_showErrorMessage('Código de status inesperado: ${response.statusCode}');
			return null;
		}
		...
		if (e.response != null && e.response?.statusCode == 400) {
			...
			final errorMessage =
			e.response?.data['message'] ?? 'Erro desconhecido';
			_showErrorMessage('Falha na criação do cliente: $errorMessage');
			return null;
		} else {
			// Tratar outros erros Dio (ex: problemas de rede)
			_showErrorMessage('Erro de rede: ${e.message}');
			return null;
		}
	} catch (e) {
...
A função \texttt{createBasicClient()} faz um pedido \texttt{POST} para o endpoint \texttt{/client/basic}, enviando os dados do cliente (nome, sobrenome e e-mail) para o backend. Caso o pedido seja bem-sucedida (código de estado 201), ela retorna os dados do cliente criado. Se houver um erro (código de estado diferente de 201 ou problemas com a rede), a função trata o erro e mostra uma mensagem de erro.
...
O desenvolvimento deste projeto foi concluído com sucesso, atendendo a todos os critérios e requisitos estabelecidos. A execução do trabalho foi feita com foco na qualidade, na documentação e na solução implementada.

Os testes realizados demonstraram que todas as funcionalidades desenvolvidas funcionam conforme o esperado. O código foi estruturado de forma modular e respeitando boas práticas de desenvolvimento, como a utilização de controladores para interação com o backend através de \textit{web requests}.  

\subtitle{Qualidade do Relatório}
O relatório produzido seguiu um padrão claro e bem organizado, apresentando detalhadamente a solução desenvolvida, os processos implementados e os resultados obtidos. Os conceitos e decisões técnicas foram documentados de forma compreensível e fundamentada.  

\subtitle{Qualidade da Solução Desenvolvida}
A aplicação demonstrou alta qualidade, refletida no seguinte:  
\begin{itemize}
	\item \textbf{Estrutura Modular e Organizada}: A aplicação segue padrões como \textit{MVC} e utiliza bibliotecas apropriadas para garantir uma arquitetura limpa e escalável.
	\item \textbf{Integração de Conceitos Avançados}: A solução implementa o LINQ e frameworks modernos para proporcionar uma implementação eficiente.
	\item \textbf{Controladores e Interações via \textit{web requests}}: A aplicação faz uso de controladores que interagem com o backend através de \textit{web requests}, promovendo uma arquitetura modular e de fácil manutenção.
	\item \textbf{Configuração de Pipelines CI/CD}: Implementação de pipelines para automação de testes e implantação contínua, utilizando ferramentas como Github Actions para garantir a qualidade e o fluxo contínuo de desenvolvimento.
	\item \textbf{Validação Rigorosa de Entradas}: A aplicação valida as entradas do utilizador de forma completa, garantindo a integridade dos dados e proporcionando um tratamento adequado de erros.
	\item \textbf{Integração entre Frontend e Backend}: Funcionalidades implementadas demonstram a integração eficiente e fluída entre o frontend e o backend, assegurando a comunicação e o processamento adequado das informações.
\end{itemize}
...
\ \\
%
Todas as ligações foram consultadas pela última vez no dia 16 de dezembro de 2024.
