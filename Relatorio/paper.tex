...
No diagrama, cada classe representa uma componente fundamental do sistema. A classe \textit{Client} (cliente) guarda informações sobre um cliente individual, como nome, e-mail e morada. Para gerir vários clientes, o sistema utiliza a classe \textit{Clients}, que funciona como uma coleção para organizar e armazenar várias instâncias da classe \textit{Client}. Esta coleção é implementada como um dicionário, permitindo acesso rápido e eficiente aos dados.

De forma semelhante, a classe \textit{Owner} (proprietário) guarda informações relacionadas com os proprietários das propriedades disponíveis no sistema. A classe \textit{Owners} funciona como uma coleção que organiza e gere as instâncias de \textit{Owner}, também utilizando um dicionário para facilitar a consulta e manipulação.

A classe \textit{Accommodation} (alojamento) representa as propriedades ou unidades disponíveis para reserva, com dados como o tipo de alojamento, a disponibilidade e o preço por noite. Cada instância de \textit{Accommodation} contém uma lista de quartos (\textit{Room}), representados pela classe \textit{Room}, que armazena detalhes específicos de cada quarto, como a sua capacidade ou características. Para gerir vários alojamentos, existe a classe \textit{Accommodations}, que é uma coleção de \textit{Accommodation} e utiliza um dicionário para manipulação eficiente.

A classe \textit{Reservation} (reserva) estabelece a ligação entre clientes e alojamentos, registando os detalhes da estadia, incluindo datas de check-in e check-out, o custo total e o estado do pagamento. Adicionalmente, cada reserva contém uma lista de pagamentos (\textit{Payment}), geridos pela classe \textit{Payment}, que regista os valores pagos, datas e métodos de pagamento. Tal como as outras coleções, a classe \textit{Reservations} guarda várias reservas utilizando um dicionário.

Para garantir uma gestão consistente e extensível, o sistema define uma interface comum, \textit{ManageableEntity}, que fornece funcionalidades partilhadas pelas classes de coleção (\textit{Clients}, \textit{Owners}, \textit{Accommodations} e \textit{Reservations}). Esta interface inclui métodos como \textit{Add}, \textit{Remove}, \textit{Import}, \textit{Export}, \textit{Save} e \textit{Load}, permitindo operações uniformes e reutilizáveis.

Por fim, o sistema integra a classe \textit{BookingManager}, uma classe face que centraliza e simplifica operações comuns do sistema. Esta classe fornece métodos como por exemplo, \textit{CreateBasicClient} (criar um cliente básico), \textit{SaveAll} (guardar todos os dados do sistema) e \textit{LoadAll} (carregar todos os dados do sistema), promovendo uma interface unificada e de fácil utilização para as operações mais frequentes.
...
\begin{lstlisting}[
	language=csh, 
	caption={Teste do Construtor da Classe \texttt{Client}},
