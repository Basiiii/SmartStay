...
Cabe destacar que nem todos os tipos numéricos são compatíveis com a CLS — por exemplo, tipos inteiros não-assinados como \texttt{uint} e \texttt{ulong} não são CLS-compliant e podem apresentar problemas de interoperabilidade em algumas linguagens. A escolha de tipos compatíveis com a CLS reforça o compromisso com a robustez e a acessibilidade do sistema em um ambiente .NET multi-linguagem.
...
\subsection{Services} A camada de \textit{Services} contém a lógica de negócio principal da aplicação. Os serviços orquestram as interações entre os modelos e os repositórios, executando operações mais complexas que envolvem várias entidades ou ações. No contexto do nosso sistema de gestão de reservas, o \textit{BookingManager} pode ser considerado um serviço, pois gerencia o processo de reservas, incluindo a criação, validação e processamento de pagamentos. Os serviços permitem que a lógica de negócios seja centralizada e desacoplada das camadas de persistência e apresentação, tornando o sistema mais fácil de testar e manter.
...
Como pode ser observado, ao receber um \texttt{ValidationErrorCode}, a classe \texttt{ValidationException} utiliza o método \texttt{GetErrorMessage} da classe \texttt{ValidationErrorMessages} para obter a mensagem correspondente ao erro, evitando a necessidade de codificar as mensagens diretamente na aplicação.
...
Cada valor da enumeração representa um tipo específico de erro de validação, como por exemplo, nome inválido, e-mail inválido, ou número de telefone inválido. Esta enumeração é usada na classe \texttt{ValidationException} para determinar o erro que ocorreu.

\subsection{Mensagens de Erro Dinâmicas}
A classe \texttt{ValidationErrorMessages} é responsável por mapear cada código de erro para uma mensagem descritiva correspondente. Em vez de codificar as mensagens diretamente nas classes de validação ou nas exceções, as mensagens são armazenadas em um dicionário, o que facilita a manutenção e a atualização das mensagens de erro.
...
namespace SmartStay.Validation
{
	public static class ValidationErrorMessages
	{
		private static readonly Dictionary<ValidationErrorCode, string> ErrorMessages = new() {
			{ ValidationErrorCode.InvalidName, "O nome deve ser uma string não vazia e não exceder 50 caracteres." },
			{ ValidationErrorCode.InvalidEmail, "O endereço de e-mail é inválido ou não corresponde ao formato exigido." }
		};
		
		public static string GetErrorMessage(ValidationErrorCode errorCode)
		{
			return ErrorMessages.TryGetValue(errorCode, out var message) ? message : "Erro de validação desconhecido.";
		}
	}
}
\end{lstlisting}

Desta forma, quando uma exceção de validação é gerada, a mensagem correspondente ao código de erro é buscada dinamicamente a partir do dicionário. Caso o código de erro não esteja mapeado, uma mensagem padrão de "Erro de validação desconhecido" será retornada, garantindo que todas as exceções sejam tratadas de forma consistente.
...
\begin{lstlisting}[
	language=csh, 
	caption={Uso da Exceção de Validação}, 
	label={lst:C}]
try
{
...
Neste exemplo, se o nome do cliente for inválido, a exceção \texttt{ValidationException} será lançada, e a mensagem de erro correspondente ao código de erro será mostrada ao utilizador.
...
A persistência de dados é um componente essencial para qualquer sistema de gestão de informações. No caso deste projeto, optei por utilizar ficheiros JSON para armazenar os dados do sistema, como clientes, acomodações e reservas. O formato JSON foi escolhido devido à sua popularidade em aplicações web e móveis, à facilidade de leitura e escrita e ao seu suporte em diversas linguagens de programação.

\subsection{Escolha do Formato de Armazenamento}

Optou-se pelo formato JSON para persistir os dados devido a vários fatores:

\begin{itemize}
	\item \textbf{Simplicidade e Legibilidade}: O JSON é um formato de texto simples, que pode ser facilmente lido por humanos e máquinas. Isso facilita tanto a depuração durante o desenvolvimento quanto a análise de dados.
	\item \textbf{Compatibilidade Universal}: O JSON é amplamente suportado por diversas plataformas e frameworks, tornando-o ideal para integração com sistemas externos e APIs.
	\item \textbf{Estrutura Flexível}: O JSON oferece uma estrutura de dados hierárquica e facilmente extensível, o que é vantajoso para representar dados complexos como clientes, reservas e acomodações de forma clara e eficiente.
	\item \textbf{Desempenho}: As bibliotecas modernas de manipulação de JSON são muito eficientes, permitindo uma rápida serialização e desserialização dos dados.
\end{itemize}
...
O método \texttt{Import} permite importar uma lista de clientes a partir de uma string JSON. Caso a string JSON seja inválida ou nula, são lançadas exceções específicas para garantir que os dados sejam válidos antes de serem inseridos na coleção. O código do método é o seguinte:
...
public void Import(string data)
{
	...
	{
		_clientDictionary[client.Id] = client; // Inserção direta para maior eficiência
	}
}
\end{lstlisting}
...
O método \texttt{Export} permite exportar a lista atual de clientes para uma string JSON, que pode ser armazenada em um ficheiro ou enviada para outros sistemas. A implementação do método é simples:
...
\begin{itemize}
	...
	\item \textbf{InvalidOperationException}: Pode ser lançada em casos em que ocorre uma operação inesperada, como ao tentar acessar dados que não foram carregados corretamente.
\end{itemize}
...
\newpage
\section{Qualidade do Código e Boas Práticas}
\label{sec:qualidadecodigo}
...
Essa abordagem foi adotada com os seguintes benefícios:
