...
\begin{itemize}
	\item \textbf{Qualidade do Código Produzido}: O código foi estruturado de forma clara e modular, com nomenclatura apropriada para ficheiros e respeito às boas práticas de desenvolvimento, incluindo o uso de bibliotecas (DLL) e conformidade com a norma CLS.
	\item \textbf{Organização e Implementação de Classes}: A implementação fez uso de interfaces, garantindo uma arquitetura flexível, modular e reutilizável.
	\item \textbf{Qualidade dos Algoritmos}: Os algoritmos desenvolvidos foram analisados quanto à sua eficiência teórica, considerando o seu comportamento em termos de complexidade \textit{Big O}, e testados com rigor por meio de testes unitários para assegurar o seu correto funcionamento.
	\item \textbf{Estruturas de Dados e Tratamento de Exceções}: As estruturas de dados foram exploradas de forma eficiente, e o tratamento de exceções foi implementado para garantir a robustez do sistema.
	\item \textbf{Tratamento de Logs}: A aplicação implementa mecanismos adequados para registro de logs, facilitando a monitorização e resolução de problemas.
	\item \textbf{Persistência de Dados}: Foi implementada a persistência de dados através do uso de ficheiros JSON e binários, garantindo a integridade e acessibilidade das informações.
	\item \textbf{Programação por Camadas}: A aplicação segue uma arquitetura por camadas, como \textit{MVC}, promovendo a escalabilidade e a separação de responsabilidades.
	\item \textbf{Exploração de Outras Valências}: Foram integrados conceitos como LINQ e a utilização do \textit{framework} Flutter para a interface do utilizador, demonstrando a aplicação de tecnologias modernas e eficientes.
	\item \textbf{Configuração de Pipelines CI/CD}: A automação de testes e implantação contínua foi implementada com sucesso, utilizando \textit{Github Actions}, garantindo qualidade e eficiência no fluxo de desenvolvimento.
\end{itemize}
